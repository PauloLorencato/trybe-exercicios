...
for (const line of listLine) {
  line.addEventListener('click', function levantadinha() {
    for (let j = 0; j < listLine.length; j += 1){
        listLine[j].classList.remove('tech');
    }
...
// com a classe 'tech'.
input.addEventListener('keyup', function inputText() {
    let value = input.value;
    document.querySelector('.tech').innerText = value;
})
